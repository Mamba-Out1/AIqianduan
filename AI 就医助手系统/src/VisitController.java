...
public class VisitController {
    ...
    public ResponseEntity<Map<String, String>> completeVisit(@PathVariable String visitId) {
        try {
            Optional<Visit> visitOpt = visitRepository.findByVisitId(visitId);
            if (visitOpt.isEmpty()) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "就诊记录不存在");
            ...
            Map<String, String> result = new HashMap<>();
            result.put("message", "就诊已完成");
            result.put("visitId", visitId);
            log.info("完成就诊成功，visitId: {}", visitId);
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            log.error("完成就诊失败，visitId: {}", visitId, e);
            return ResponseEntity.internalServerError().build();
        }
    }
